...
const walletClientUsage = async(): Promise<void> => {
  ...
  try {
    ...
    await walletClient.sendSignedPayloadAsync({id: 0, signature: `0x${u8aToHex(signature)}`});
    await walletClient.sendSignedRawAsync({id: 0, signature: `0x${u8aToHex(signature)}`});
